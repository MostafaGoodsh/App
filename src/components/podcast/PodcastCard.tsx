...
const PodcastCard = ({ card }: { card?: HomePageCard }) => {
  const { getSetting } = useTypography();
  const { language, t } = useLanguage();
  const homeSetting = getSetting(getCardTypographySectionKey(card?.card_type ?? "podcast")) || getSetting("home_cards") || getSetting("general");

  const isArabic = language === "ar" || language === "both";

  const title = (!isArabic && card?.title_en) 
    ? card.title_en 
    : (card?.title || t("بودكاست وراديو", "Podcast & Radio"));
  const desc = (!isArabic && card?.description_en) 
    ? card.description_en 
    : (card?.description ?? t("استمع للمحتوى الصوتي والبودكاست", "Listen to audio content and podcasts"));

  const backgroundImage = card?.background_image || "/lovable-uploads/podcast-radio-bg.jpg";
  ...
  return (
    <Link to={card?.route_path || "/podcast"} className="group">
      <article
      ...
      >
        ...
        <div className="relative p-8 min-h-[280px] md:min-h-[320px] flex flex-col justify-end items-stretch bg-gradient-to-t from-background/90 via-background/60 to-transparent">
          ...
          <p className="font-cairo text-xs text-white/70 mt-2 w-full" style={descStyle}>
            {t("🎧 بودكاست • راديو • موسيقى", "🎧 Podcast • Radio • Music")}
          </p>
