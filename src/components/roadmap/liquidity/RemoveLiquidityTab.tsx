...
import { ArrowUpCircle, Info, Lock, AlertTriangle } from 'lucide-react';
    ...
    return (
      <Card className="bg-card/80 border-primary/20">
        <CardContent className="p-8 text-center">
          ...
          <p className="font-cairo text-sm text-muted-foreground">لا توجد مراكز نشطة للسحب</p>
          <p className="text-[10px] text-muted-foreground/60 mt-1">أضف سيولة أولاً من تبويب "إضافة"</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {activePositions.length > 1 && (
        <div className="space-y-2">
          <Label className="font-cairo text-xs">اختر المركز</Label>
          {activePositions.map(pos => (
            <Card
              key={pos.id}
              onClick={() => setSelectedPositionId(pos.id)}
              className={`cursor-pointer transition-all ${
                (selectedPositionId || activePositions[0]?.id) === pos.id
            ...
            >
              <CardContent className="p-3 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  {pos.is_staked && <Badge variant="secondary" className="text-[9px]"><Lock className="w-2.5 h-2.5 mr-1" />مقفل</Badge>}
                  <Badge variant="outline" className="text-[9px]">${pos.earned_rewards.toFixed(2)} ربح</Badge>
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Card className="bg-card/80 backdrop-blur-sm border-primary/20">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <span className="font-cairo">سحب سيولة</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          {isLocked && (
            <Alert variant="destructive">
              ...
              <AlertDescription className="text-xs font-cairo">
                هذا المركز مقفل حتى {new Date(selectedPosition!.stake_unlock_at!).toLocaleDateString('ar')}
              </AlertDescription>
            </Alert>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <Button
                variant="ghost"
                size="sm"
                className="text-[10px] h-5 text-primary"
            ...
            <Input
              ...
              max={selectedPosition?.deposited_amount}
              placeholder="0.00"
            ...
            {selectedPosition && (
              <p className="text-[9px] text-muted-foreground mt-1">
                المتاح: ${selectedPosition.deposited_amount.toFixed(2)}
              </p>
            )}
          </div>
              ...
              <AlertDescription>
                <div className="space-y-1 text-xs font-cairo">
                  <div className="flex justify-between">
                    <span>رسوم السحب ({ap.fee_percentage}%)</span>
          ...
          <Button
            ...
            disabled={numAmount <= 0 || !!isLocked || submitting}
            variant="destructive"
            className="w-full font-cairo"
          >
            {submitting ? 'جاري السحب...' : 'سحب سيولة'}
          </Button>
        </CardContent>
      </Card>
    </div>
