...
-- Audit function for wallet security access
CREATE OR REPLACE FUNCTION public.log_wallet_security_access()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
-- Trigger for wallet security access logging
DROP TRIGGER IF EXISTS wallet_security_access_trigger ON public.wallet_security;
CREATE TRIGGER wallet_security_access_trigger
  AFTER SELECT ON public.wallet_security
  FOR EACH ROW
  EXECUTE FUNCTION public.log_wallet_security_access();

-- Fix 4: Enhanced identity verification security
-- Add policy to restrict document URL access
CREATE POLICY "Restrict document URL access" 
ON public.identity_verification 
FOR SELECT 
USING (
  -- Only user or admin can see full data
  auth.uid() = user_id OR public.is_admin(auth.uid())
);

-- Create secure view for admin access to identity data
...
GRANT SELECT ON public.identity_verification_admin_secure TO authenticated;